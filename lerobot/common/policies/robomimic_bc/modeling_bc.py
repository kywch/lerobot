...
from lerobot.common.policies.robomimic_bc.configuration_bc import BCConfig
...
class BCPolicy(
    ...
    def _process_input_shape_config(self):
        ...
        for key in self.config.input_shapes:
            ...
            if "image" in key:
                # image_keys.append(lerobot_key)  # XCXC -- see also 
                # robomimic.envs.env_robosuite.py # L 199, get_observation()
        ...
        if rm_obs_utils.OBS_KEYS_TO_MODALITIES is None:
            rm_obs_utils.initialize_obs_modality_mapping_from_dict(modality_mapping={"rgb": image_keys})
        else:
    ...
    def select_action(self, obs_dict: dict[str, Tensor]) -> Tensor:
        ...
        if self._rnn_hidden_state is None or self._rnn_counter % self._rnn_horizon == 0:
            ...
            self._rnn_hidden_state = self.model.get_rnn_init_state(batch_size=batch_size, device=self.device)
        ...
        action, self._rnn_hidden_state = self.model.forward_step(
            obs_to_use, rnn_state=self._rnn_hidden_state)  # NOTE: not supporting goal_dict=goal_dict
        return action
